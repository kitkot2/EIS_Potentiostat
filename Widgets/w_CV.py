...
class CVWidget(QWidget):
    ...
    measurement_finished = Signal(list, list)  # voltages, currents
    ...
    def __init__(self, ad5941_instance=None, parent=None):
        ...
        # Default parameters
        self.start_mv = -200.0      # mV
        self.stop_mv = 200.0        # mV
        self.step_mv = 2.0          # mV
        self.rate_mvs = 100.0       # mV/s
        self.cycles = 1             # number of cycles
        self.tia_rf_index = 1       # 0-7
        self.pga_gain_index = 1     # 0-4
        self.bias_mv = 0.0          # bias voltage (mV)
        self.offset_mv = 0.0        # offset voltage (mV)
    ...
    def _get_settings_path(self):
        ...
        home = Path.home()
        return home / ".hunstat2_cv_settings.json"
    ...
    def load_settings(self):
        ...
        try:
            ...
            self.start_mv = data.get('start_mv', self.start_mv)
            self.stop_mv = data.get('stop_mv', self.stop_mv)
            self.step_mv = data.get('step_mv', self.step_mv)
            self.rate_mvs = data.get('rate_mvs', self.rate_mvs)
            self.cycles = data.get('cycles', self.cycles)
    ...
    def save_settings(self):
        ...
        settings_path = self._get_settings_path()
        data = {
            'start_mv': self.start_mv,
            'stop_mv': self.stop_mv,
            'step_mv': self.step_mv,
            'rate_mvs': self.rate_mvs,
            'cycles': self.cycles,
    ...
    def _create_widgets(self):
        ...
        self.param_group = QGroupBox("CV Parameters")
        ...
        # Right panel: plots
        self.plot_group = QGroupBox("Cyclic Voltammogram")
    ...
    def _create_parameter_controls(self, parent_layout):
        ...
        # Start potential
        grid.addWidget(QLabel("Start (mV):"), 0, 0)
        self.start_edit = QLineEdit(str(self.start_mv))
        self.start_edit.setMaximumWidth(INPUT_WIDTH)
        grid.addWidget(self.start_edit, 0, 1)

        # Stop potential
        grid.addWidget(QLabel("Stop (mV):"), 1, 0)
        self.stop_edit = QLineEdit(str(self.stop_mv))
        self.stop_edit.setMaximumWidth(INPUT_WIDTH)
        grid.addWidget(self.stop_edit, 1, 1)

        # Step
        grid.addWidget(QLabel("Step (mV):"), 2, 0)
        self.step_edit = QLineEdit(str(self.step_mv))
        self.step_edit.setMaximumWidth(INPUT_WIDTH)
        grid.addWidget(self.step_edit, 2, 1)

        # Scan rate
        grid.addWidget(QLabel("Rate (mV/s):"), 3, 0)
        self.rate_edit = QLineEdit(str(self.rate_mvs))
        self.rate_edit.setMaximumWidth(INPUT_WIDTH)
        grid.addWidget(self.rate_edit, 3, 1)

        # Cycles
        grid.addWidget(QLabel("Cycles:"), 4, 0)
        self.cycles_edit = QLineEdit(str(self.cycles))
        self.cycles_edit.setMaximumWidth(INPUT_WIDTH)
        grid.addWidget(self.cycles_edit, 4, 1)
        ...
        # Info label
        self.param_info = QLabel("Set parameters and press 'Run CV Scan'")
        self.param_info.setStyleSheet("color: gray;")
        grid.addWidget(self.param_info, 10, 0, 1, 2)
    ...
    def _connect_signals(self):
        ...
        self.run_button.clicked.connect(self._run_cv_scan_thread)
    ...
    def _update_parameters(self):
        ...
        try:
            self.start_mv = float(self.start_edit.text())
            self.stop_mv = float(self.stop_edit.text())
            self.step_mv = float(self.step_edit.text())
            self.rate_mvs = float(self.rate_edit.text())
            self.cycles = int(self.cycles_edit.text())
    ...
    @Slot()
    def _run_cv_scan_thread(self):
        """Start CV measurement in a separate thread."""
        ...
        thread = threading.Thread(target=self._run_cv_scan, daemon=True)
        thread.start()

    def _run_cv_scan(self):
        """Perform CV measurement using AD5941."""
        try:
            # Configure device
            self.ad5941.set_tia_rf(self.tia_rf_index)
            self.ad5941.set_pga_gain(self.pga_gain_index)
            self.ad5941.set_vbias_mv(self.bias_mv)
            self.ad5941.set_offset_mv(self.offset_mv)
            self.ad5941.set_use_variable_gain(self.use_variable_gain)
            # Ensure ASCII mode for CV
            self.ad5941.set_seeedstat_mode(True)

            # Run measurement
            cv_data = self.ad5941.run_cv_scan(
                start_mv=self.start_mv,
                stop_mv=self.stop_mv,
                step_mv=self.step_mv,
                rate_mvs=self.rate_mvs,
            ...
            self.data_queue.put(('data', (voltages, currents)))
    ...
    @Slot(list, list)
    def _on_measurement_finished(self, voltages, currents):
    ...
    def _update_plot(self):
        ...
        self.ax.plot(self.voltages, self.currents, 'b-', linewidth=1.5)
        self.ax.set_xlabel("Potential (mV)")
        self.ax.set_ylabel("Current (μA)")
        ...
        # Add arrow to show scan direction (first half)
        if len(self.voltages) > 1:
            mid = len(self.voltages) // 2
            if mid + 10 < len(self.voltages):
                self.ax.annotate('', xy=(self.voltages[mid+10], self.currents[mid+10]),
                               xytext=(self.voltages[mid], self.currents[mid]),
